...
export default function DashboardSidebar({ brandTitle, brandSubtitle, links, footer }: DashboardSidebarProps) {
  ...
  return (
    <>
      <div className="md:hidden flex items-center justify-between px-4 py-3 border-b border-neutral-200/70 bg-white">
        <div className="flex items-center gap-2">
          <div className="h-2 w-2 rounded-full bg-gradient-to-br from-yellow-400 to-yellow-600" />
          <p className="font-bold text-sm text-neutral-900">{brandTitle}</p>
        </div>
        <button
      ...
      <aside
        className={`fixed md:static inset-y-0 left-0 z-50 w-72 md:w-64 md:min-h-screen overflow-y-auto
          border-r border-neutral-200/70 p-6 space-y-8 bg-white shadow-2xl md:shadow-none
        ...
        <div className="flex items-start justify-between">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <div className="h-2 w-2 rounded-full bg-gradient-to-br from-yellow-400 to-yellow-600" />
              <p className="font-bold text-neutral-900">{brandTitle}</p>
            </div>
            {brandSubtitle && <p className="text-xs text-neutral-400 pl-4">{brandSubtitle}</p>}
          </div>
          <button
            onClick={() => setOpen(false)}
